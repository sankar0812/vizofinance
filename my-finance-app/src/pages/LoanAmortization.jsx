...
const LoanAmortization = ({ loanAmount, annualInterestRate, loanTermMonths, days }) => {
  ...
  useEffect(() => {
    const calculateSchedule = () => {
      ...
      if (mode === 'monthly') {
        if (annualInterestRate < 0 || loanTermMonths <= 0) {
          setScheduleData([]);
          return;
        }
        ...
        const principalPerDay = loanAmount / days;
        const interestPerDay = principalPerDay * 0.10; // 10% of principal
        ...
        for (let day = 1; day <= days; day++) {
          balance -= principalPerDay;
          totalPaid += dailyPayment;

          data.push({
            period: day,
            payment: parseFloat(dailyPayment.toFixed(2)),
            principalPaid: parseFloat(principalPerDay.toFixed(2)),
            interestPaid: parseFloat(interestPerDay.toFixed(2)),
            endingBalance: parseFloat((balance < 0 ? 0 : balance).toFixed(2)),
            cumulativePaid: parseFloat(totalPaid.toFixed(2)),
          });
        }
    ...
    calculateSchedule();
  }, [loanAmount, annualInterestRate, loanTermMonths, days, mode]);
  ...
  if (scheduleData.length === 0) {
    return (
      <Paper sx={{ p: 3, mt: 3, textAlign: 'center' }}>
        <Typography variant="body2" color="text.secondary" fontStyle="italic">
          Please enter valid inputs to view the amortization schedule.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper sx={{ p: 3, mt: 3 }}>
      <Typography variant="h5" fontWeight="bold" gutterBottom borderBottom={1} pb={1}>
        {mode === 'monthly' ? 'Monthly' : 'Daily'} Loan Amortization Schedule
      </Typography>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Box sx={{ height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={scheduleData} margin={{ top: 0, right: 0, left: 30, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" />
        ...
        <Table stickyHeader>
          <TableHead>
            <TableRow>
              ...
              <TableCell>{mode === 'monthly' ? 'Monthly Payment' : 'Daily Payment'}</TableCell>
              <TableCell>Principal Paid</TableCell>
              <TableCell>Interest Paid</TableCell>
              <TableCell>Ending Balance</TableCell>
              {mode === 'daily' && <TableCell>Cumulative Paid</TableCell>}
            </TableRow>
          </TableHead>
          <TableBody>
            {scheduleData.map((row) => (
              <TableRow key={row.period} hover>
                ...
                <TableCell>{formatINR(row.interestPaid)}</TableCell>
                <TableCell>{formatINR(row.endingBalance)}</TableCell>
                {mode === 'daily' && <TableCell>{formatINR(row.cumulativePaid)}</TableCell>}
              </TableRow>
            ))}
          </TableBody>
        </Table>
